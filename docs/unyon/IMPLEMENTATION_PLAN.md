...
Build a modular monolith: one Next.js application and one PostgreSQL database, organized by product feature. Keep route handlers and Server Actions thin. Each feature exposes a small server interface that hides validation, authorization, transactions, auditing, and persistence.

The first delivery is a deployed vertical slice, not a collection of disconnected screens:
...
flowchart TD
    ...
    Z --> P[(Prisma / Supabase PostgreSQL)]
    D --> P
...
- **Prisma/PostgreSQL** owns domain state, authority, constraints, and audit records.
...
│   ├── database/                # Prisma client and transaction setup
...
Each feature exposes its external seam from `server/index.ts`. Its implementation, Prisma queries, policy details, and adapters remain private. Features may import another feature's public interface, never its internals. Browser modules cannot import `server-only`, Prisma, Firebase administration, or R2 bindings.
...
4. Resolve canonical resource ownership from PostgreSQL.
...
PostgreSQL-only mutations perform authorization, mutation, and audit writes in one transaction. Missing and forbidden scoped resources return the same public error. Raw dependency errors never cross the interface.
...
Add database constraints for valid owner combinations, unique active role assignments, unique evaluation responses, revision numbers, and allowed lifecycle values. Use raw SQL migrations where PostgreSQL partial indexes or check constraints exceed Prisma's schema surface.
...
- Scaffold Next.js, strict TypeScript, Tailwind, shadcn/ui, Vinext, Wrangler, Prisma, and test tooling.
- Configure local, preview, and production environment validation.
- Deploy a Worker proving session creation/verification, one Prisma read/write through Supabase's transaction pooler, one private R2 upload/download, and one allow/deny authorization case.
- Measure CPU time and bundle compatibility.

**Exit:** the deployed vertical infrastructure proof passes. Use OpenNext if Vinext fails a required capability; use Identity Platform REST/JWT verification if Firebase Admin fails.

### Phase 1 — Access and directory

- Create the initial schema, migrations, audit model, bootstrap command, and seed data.
...
- **Integration tests:** run real Prisma migrations and implementations against disposable PostgreSQL.
...
pnpm test:integration    # Disposable PostgreSQL tests
pnpm test:e2e            # Playwright journeys
pnpm prisma:migrate      # Apply development migrations
pnpm backup              # Encrypted logical database backup
...
| Supabase Free 500 MB database | Warn at 70%, require action at 85%, index measured queries, archive only through approved policy. |
| Supabase inactivity pausing | Daily legitimate health query and maintenance alert; disclose pilot availability. |
| R2 and PostgreSQL non-atomic writes | Pending/available/failed object states and compensating cleanup. |
...
| Authorization drift | Typed intents, protected-operation factories, private Prisma imports, and exhaustive policy tests. |
...
- PostgreSQL constraints defend critical invariants independently of UI checks.
